...
function getTransformValue (node) {
  ...
  const transformValues = transform
    ...
    .split(')')[0]
    .split(',')
  return +transformValues[transformValues.length - 1]
}
...
export default class Body extends PureComponent {
  ...
  getWrap = () => {
    const wrap = document.querySelector('#scrolling-container')
    if (wrap) { this.getRootNode = () => wrap }
    return wrap
  }
  ...
  componentDidMount () {
    ...
    this.rootScroll = new BScroll(rootNode, {
      scrollY: true,
      probeType: 2,
    })

    this.bodyScroll = new BScroll(body, {
      scrollY: true,
      click: true,
      bounce: false,
      probeType: 2,
    ...
    this.rootScroll.on('scroll', this.handleRootScroll)
    this.bodyScroll.on('scroll', this.handleBodyScroll)
  }

  handleRootScroll = ({ y: transition }) => {
    if (transition > THRESHOLD) {
      this.bodyScroll.disable()
    }

    if (transition <= THRESHOLD) {
      this.rootScroll.disable()
      this.rootScroll.stop()
      this.bodyScroll.enable()
    }
  }

  handleBodyScroll = ({ y: transition }) => {
    if (transition === 0) {
      this.rootScroll.enable()
      this.bodyScroll.disable()
    }
  }
